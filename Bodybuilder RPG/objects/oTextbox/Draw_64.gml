...
// draw the text
draw_text_color(margin, CAM_HEIGHT - margin, name == "" ? drawStr : $"{name}: {drawStr}", c_white, c_white, c_white, c_white, 1);
...
if (showPrompt) {
    ...
    var promptWidth = 200;
    var promptHeight = 42;
    var c1 = #2b7eb5, c2 = #81b7db, c3 = c1, c4 = #1f3747;
    ...
    if (promptScale == 1) {
        var drawPromptText = function(promptX, promptY, txt, index, selection) {
            var cY = c_yellow;
            var yDist = 10;
            draw_text_color(promptX, index == 0 ? promptY - yDist : promptY + yDist, txt, selection == index ? cY: c_white, selection == index ? cY: c_white, selection == index ? cY: c_white, selection == index ? cY: c_white, 1);
            return;
        }
