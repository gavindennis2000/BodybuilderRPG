...
if (next) {
	if (numberOfTexts <= 1 and prompt != false) {
		...
		var colorSelection1 = selection == 0 ? c_yellow : c_white;
		var colorSelection2 = selection == 1 ? c_yellow : c_white;
		...
		draw_sprite_ext(sprNextGlow, 0, camX+240, camY + 64-2, 1, 1, image_angle, image_blend, glowAlpha);
		draw_sprite_ext(sprNext, 0, camX+240, camY + 64-2, 1, 1, image_angle, image_blend, nextAlpha);
	}
}
...
if (npcID != -1) {
	...
	draw_text_border(camX + 4, camY - 2, string_upper(name), c_white, 1, c_gray);
}